...
print "Ingresar el archivo de texto con la secuencia completa que contiene el target de interés solamente con los nucleótidos: ";
...
while($bul==0 & length($adn) > 200){
	...
	while($pero==0){
		...
		if ($inicio > 28){
			...
			if ($inicio>=0){
				print"No se encuentra un forward, ingrese otra posición o 0 para salir: ";
				$inicio=<STDIN>; $d=0;
				if($inicio==0){
					exit;
				}
